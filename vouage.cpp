...
#include <cmath>
#include <QMessageBox>
int r,ris=0,prirost,xcenter,ycenter,mins,leng,s;
double gradus=360;
int ways[10][10];
bool *was;
int *way, *minway;
...
void Vouage::Poisk(int l, int i){
    ...
    if(!b){
        bool minus=0;
        for (int k=0;k<r;k++) if (way[k]==-1) minus=1;
        if (!minus&& leng<mins){
            mins=leng;
            for (int z=0;z<r;z++){
                minway[z]=way[z];
...
void Vouage::on_go_clicked()
{
    ...
    way = new int [r];
    minway = new int [r];
    for (int i=0;i<r;i++) {minway[i]=-1;way[i]=-1;}
    for (int i=0;i<r;i++) Poisk(0,i);
    for (int i=0;i<r;i++) if (minway[i]==-1) notway=1;
    if (notway||mins==10000){
